...
void SimulationTests::createObjectListList() {
    ...
    test::Test* remove_test = list->addTest(
        "remove",
        {
            add_test,
            remove_joint_test
        ...
        [&](test::Test& test) {
            ...
            tAssert(tCompare(simulation.getAllSize(), 2));
            tAssert(tCompare(simulation.getTopSize(), 2));
            tCheck(simulation.getFromAll(0) == box0);
            tAssert(tCompare(box0->getChildren().size(), 0));
            tCheck(box0->getParent() == nullptr);
            tCheck(simulation.getFromAll(1) == box2);
            tAssert(tCompare(box2->getChildren().size(), 0));
            tCheck(box2->getParent() == nullptr);
            tAssert(tCompare(simulation.getJointsSize(), 0));
        }
    );
    test::Test* remove_with_children_test = list->addTest(
        ...
        [&](test::Test& test) {
            ...
            tAssert(tCompare(simulation.getTopSize(), 1));
            tCheck(simulation.getFromAll(0) == box0);
            tAssert(tCompare(box0->getChildren().size(), 0));
            tCheck(box0->getParent() == nullptr);
            tAssert(tCompare(simulation.getJointsSize(), 0));

...
public:
	...
	T* add(const T& value);
	T* add(const T* ptr);
	T* add(dp::DataPointerUnique<T> value);
	bool insert(const std::vector<T*>::const_iterator& where, T* ptr);
	bool insert(const std::vector<T*>::const_iterator& where, dp::DataPointerUnique<T> value);
	template<std::incrementable TIter>
	size_t insert(const std::vector<T*>::const_iterator& where, const TIter& first, const TIter& last);
	...
	dp::DataPointerUnique<T>& getUptr(size_t index);
	ptrdiff_t getIndex(const T* value) const;
...
private:
	...
	std::vector<dp::DataPointerUnique<T>> uptrs;
...
inline CompVectorUptr<T, TCmp>::CompVectorUptr(const std::initializer_list<T>& list) {
	for (const T& value : list) {
		dp::DataPointerUnique<T> uptr = dp::make_data_pointer<T>("CompVectorUptr entry", value);
		add(std::move(uptr));
	}
}
...
inline CompVectorUptr<T, TCmp>::CompVectorUptr(const std::vector<T*>& vec) {
	for (const T* value : vec) {
		dp::DataPointerUnique<T> uptr = dp::DataPointerUnique<T>(const_cast<T*>(value));
		add(std::move(uptr));
	}
}
...
inline T* CompVectorUptr<T, TCmp>::add(const T& value) {
	dp::DataPointerUnique<T> uptr = dp::make_data_pointer<T>("CompVectorUptr entry " + std::to_string(size()), value);
	T* ptr = uptr.get();
	comp.add(ptr);
...
inline T* CompVectorUptr<T, TCmp>::add(const T* ptr) {
	...
	if (added) {
		dp::DataPointerUnique<T> uptr = dp::DataPointerUnique<T>("CompVectorUptr entry", nptr);
		uptrs.push_back(std::move(uptr));
		return nptr;
	}
	return nullptr;
...
template<typename T, typename TCmp>
inline T* CompVectorUptr<T, TCmp>::add(dp::DataPointerUnique<T> value) {
	T* ptr = value.get();
...
inline bool CompVectorUptr<T, TCmp>::insert(const std::vector<T*>::const_iterator& where, T* ptr) {
	if (contains(ptr)) {
		...
		dp::DataPointerUnique<T> uptr("CompVectorUptr entry", ptr);
		return insert(where, std::move(uptr));
	}
}

template<typename T, typename TCmp>
inline bool CompVectorUptr<T, TCmp>::insert(const std::vector<T*>::const_iterator& where, dp::DataPointerUnique<T> value) {
...
inline size_t CompVectorUptr<T, TCmp>::insert(const std::vector<T*>::const_iterator& where, const TIter& first, const TIter& last) {
	...
	size_t inserted_count = 0;
	while (iter_other != last) {
		if (insert(comp.begin() + offset, std::move(const_cast<dp::DataPointerUnique<T>&>(*iter_other)))) {
			inserted_count++;
			offset++;
		}
		iter_other++;
...
template<typename T, typename TCmp>
inline dp::DataPointerUnique<T>& CompVectorUptr<T, TCmp>::getUptr(size_t index) {
	return uptrs[index];
}
...
inline void CompVectorUptr<T, TCmp>::clear() {
	uptrs = std::vector<dp::DataPointerUnique<T>>();
	comp.clear();
}
